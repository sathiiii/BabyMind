...
class VisionEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Predicts C concept logits from a (B, E) image feature.
        self.num_concepts = self.args.get("num_concepts", 22)

        # --- isolate RNG so later modules get baseline draws ---
        cpu_state = torch.random.get_rng_state()
        cuda_state = (torch.cuda.get_rng_state_all()
                      if torch.cuda.is_available() else None)

        # create concept head ONLY if neurosym is enabled
        self.concept_head = None
        if self.args.get("neurosym", False):
            self.concept_head = nn.Linear(self.embedding_dim, self.num_concepts, bias=True)
            # pick any init you like for the head itself; it will not affect others
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--num_concepts", type=int, default=22,
                            help="number of concepts for neuro-symbolic head")

    def concept_logits(self, image_features, image_feature_map=None):
        """
        image_features: (B,E) if embedding_type='flat'
                        (B,E,H,W) if embedding_type='spatial'
        returns: (B, C) logits
        """
        if self.concept_head is None:
            raise RuntimeError("concept_head is not initialized (neurosym disabled).")
        if image_features.dim() == 4:  # spatial
            pooled = image_features.mean(dim=(2, 3))
        else:
            pooled = image_features
        return self.concept_head(pooled)

    def forward(self, x, requires_grad_for_fmap: bool = False):
        """
        ...
        Args:
            ...
            requires_grad_for_fmap:
                If True, mark the last conv feature map so that gradients
                are available for GradCAM-style visualizations. This does
                not change whether the CNN weights are trainable; that is
                still controlled by finetune_cnn.
        """
        # If we are going to use GradCAM and gradients are globally enabled,
        # make sure the input requires grad so that intermediate feature maps
        # also carry gradients even when the CNN weights are frozen.
        ...
        if hasattr(self, "vit_dino") and self.vit_dino:
            ...
            if self.embedding_type == "spatial":
                # for spatial embedding we keep the penultimate block as fmap
                layer = self.model[-2]
            else:
                # for flat embedding we keep the last ResNeXt block (layer4)
                layer = self.model.layer4

            # Hook will capture the conv feature map as image_feature_map
...
class MultiModalModel(nn.Module):
    ...
    def encode_image(self, image):
        # When vm_log_gradcam is enabled (set in the Lightning wrapper),
        # ask the vision encoder to keep gradients for the last conv feature
        # map so that GradCAM can be computed with respect to the
        # image–text contrastive score.
        track_gradcam = bool(self.args.get("vm_log_gradcam", False))
        try:
            image_features, image_feature_map = self.image_embed(
                image, requires_grad_for_fmap=track_gradcam)
        except TypeError:
            # Backwards compatibility for custom encoders without the extra arg.
    ...
    def compute_gradcam(
        ...
        """
        Compute GradCAM heatmaps for the contrastive image-text score.

        Args:
            logits_per_image: (B, B) similarity logits.
            image_feature_map: (B, C, Hf, Wf) conv feature map from the vision encoder.
            target_indices: optional (B,) tensor with the index in the text batch
                to treat as the "positive" for each image. Defaults to the diagonal
                (matching pairs).

        Returns:
            cam: (B, 1, IMAGE_H, IMAGE_W) GradCAM maps in [0, 1], or None if disabled.
        """
        if (not self.args.get("vm_log_gradcam", False)) or image_feature_map is None:
            return None
        ...
        if target_indices is None:
            target_indices = torch.arange(B, device=device)

        # scalar score = sum of positive logits
        scores = logits_per_image[torch.arange(B, device=device), target_indices]
        score = scores.sum()

        # gradient of score w.r.t. feature map
        ...
        cam = (weights * image_feature_map).sum(dim=1, keepdim=True)  # (B, 1, Hf, Wf)
        cam = F.relu(cam)
